...
print("Bienvenue dans mon escape room. Essaie de trouver la sortie. Il y a 7 piece (0-6)")
...
while inGame:
    ...
    if room_number == "0":
        ...
        if "key" in items:
            print("Vous ouvrez la porte est vous trouvez un calcul : 2 x 2")
        else:
        ...
        if "viande" in items:
            ...
            print("Tu as trouver un bout de viande cru dans un frigo")
            items.append("viande")
    elif room_number == "6":
        print("il y a un lion et deriere in numero")
        lion = input("Do you want to push the lion? (yes/no)")
        if lion == "yes":
            if "viande" not in items:
                print("""tu pousse le lion mais il te mange""")
                inGame = False
                lion_eat = True
            else:
                print("""Tu lance le bout de viande dans le coin de la piece et le lion saute dessus.
                      Il y a un numero: 6""")
                items.append("6 derriere le lion")
